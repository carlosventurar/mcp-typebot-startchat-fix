...
async function main() {
  ...
  const toolsMap = new Map<string, {
    ...
    ['startChat', {
      func: startChat,
      description: 'Obtiene información para iniciar un chat con un Typebot (devuelve URL). Requiere botId, opcional chat.context',
      schema: z.object({
        botId: z.string().min(1, "El campo 'botId' es obligatorio."),
        chat: z.object({
    ...
    ['getChatUrl', {
      func: getChatUrl,
      description: 'Obtiene la URL directa para acceder a un bot. Requiere botId, opcional prefilledVariables',
      schema: z.object({
        botId: z.string().min(1, "El campo 'botId' es obligatorio."),
        prefilledVariables: z.record(z.any()).optional(),
      }),
    }],
  ]);
